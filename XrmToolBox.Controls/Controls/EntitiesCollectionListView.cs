...
{
    /// <summary>
    /// Implements the Bound List View and allows displaying a list of Entities in a ListView
    /// </summary>
    public partial class EntitiesCollectionListView : BoundListViewControl
    {
        ...
        private List<AttributeMetadata> _attribMetadata;
        #endregion
        ...
        public EntitiesCollectionListView()
        {
            ...
            // no need to check... always an Entity~
            ListItemType = typeof(Entity);
        ...
        private void RetrieveEntities(QueryBase query)
        {
            ...
            try
            {
                ...
                worker.DoWork += (w, e) => {
                    ...
                    List<AttributeMetadata> attribs = null;
                    attribs = CrmActions.RetrieveEntityAttributes(Service, records.EntityCollection.EntityName, true);
        ...
        public override void LoadData<T>(List<T> items)
        {
            ...
            // call the shared internal load data method 
            LoadData(AllEntities, null);
        }

        private void LoadData(List<Entity> entities, List<AttributeMetadata> attribMeta)
        {
            _allItems = entities.Select(i => i as object).ToList();

            _attribMetadata = attribMeta;

            // use the attribute metadata to set up the cols
            SetUpListViewColumns(entities, _attribMetadata);
        ...
        /// 
        /// </summary>
        internal override void PopulateListView() 
        {
            ...
            if (entities != null && entities?.Count > 0)
            {
                ...
                {
                    var col = Columns[0];
                    var colDef = Columns[0].Tag as ListViewColumnDef;
                    var attName = colDef.Name;
                    ...
                    var lvItem = new ListViewItem()
                    {
                        ...
                        ImageIndex = 0,
                        StateImageIndex = 0,
                        Text = GetEntityValue(record, attName, _attribMetadata.Where(a => a.LogicalName == attName).FirstOrDefault()),
                    ...
                    {
                        colDef = Columns[i].Tag as ListViewColumnDef;
                        attName = colDef.Name;

                        var colVal = GetEntityValue(record, attName, _attribMetadata.Where(a => a.LogicalName == attName).FirstOrDefault());
            ...
            try
            {
                ...
                else if (entity.Contains(attribName) && entity[attribName] != null)
                {
                    ...
                    if (ShowFriendlyNames)
                    {
                        ...
                        if (!Utility.ValueTypeIsFriendly(value) && (attrib != null))
                        {
        ...
        internal void SetUpListViewColumns(List<Entity> entities, List<AttributeMetadata> attributes = null)
        {
            ...
            if (attributes == null)
            {
                attributes = CrmActions.RetrieveEntityAttributes(Service, entities[0].LogicalName, true);
            }
